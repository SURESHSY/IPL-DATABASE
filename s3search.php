...
echo "";
$s_number=$_POST['season'];


$sql="SELECT *  FROM I_seasons where  ssnumber='$s_number'";
$result=$conn->query($sql);
if($result->num_rows >0)
{
	echo  "<table><tr><th>SEASON</th><th>POT</th><th>ORANGE_CAP</th><th>SPONSORS</th>
	<th>PURPLE_CAP</th><th>WINNERS</th></tr>";
	while($row=$result->fetch_assoc()){
		echo "<tr><td>".$row["ssnumber"]."</td>".
		"<td>".$row["POT"]."</td>".
		"<td>".$row["Orange_cap"]."</td>".
		"<td>".$row["sponsors"]."</td>".
		"<td>".$row["purple_cap"]."</td>".
		"<td>".$row["winners"]."</td>"."</tr>";
	}
}
else{
	echo "0 results";
